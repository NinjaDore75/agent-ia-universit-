...
batiments_universite = {
    ...
    "Lefebvre": {"lettre": "D", "ufr": "UFR de Sociologie", "localisation" : "ouest", "ufr":"Sciences Sociales et Administration"},
    ...
    "Allais": {"lettre": "G", "localisation" : "sud", "ufr":"Sciences Economiques Gestion Mathématiques et Informatiques"},  # UFR non précisée
    "Omnisport": {"lettre": "H", "localisation" : "sud"},  # Centre sportif
    "Éphémère 1": {"lettre": "M","ufr" : "Direction des affaires logistique et optimisation des environnements du travail", "localisation":"sud"},  # Temporaire, sans UFR précise
    "Maison de l'Étudiant": {"lettre": "MDE", "localisation":"centre, sud ouest"},
    "Ricoeur": {"lettre": "L", "localisation" : "est", "ufr":"Philosophie Information-Communication Langage Littérature Arts du Spectacle"},  # UFR non précisée
    "Gymnase": {"lettre": "I"},  # Sport
    "Maier": {"lettre": "V", "localisation":"nord", "ufr":"Langues et Cultures Etrangères"},  # UFR non précisée
    "Milliat": {"lettre": "S", "ufr":"Sciences Techniques des Activités Physiques et sportives", "localisation":"Nord"},  # UFR non précisée
...
def extract_keywords(question):
    ...
    if "association" in question or "club" in question or "étudiant" in question:
    ...
    if "ufr" in question_lower:
        ...
        for batiment, info in batiments_universite.items():
            if "ufr" in info and info["ufr"].lower() in question_lower:
                ...
                keywords.append(batiment.lower())
            if "ufr" in info and any(discipline in question_lower for discipline in ["philo", "socio", "psycho", "segmi", "lettres", "politique", "droit", "eco"]):
    ...
    if is_association_question:
        keywords = [word for word in keywords if word not in ["contact", "telephone", "email"]]  # Enlever les mots-clés de contact
...
def get_main_subject(question):
    ...
    important_words = [word for word in keywords if len(word) > 3 and word not in ["asso", "association", "université", "campus"]]
...
def answer_university_contact_question(question):
    ...
    # Vérification si la question concerne l'université
    university_terms = ["université", "paris nanterre", "univ", "fac", "campus"]
    if not any(term in question_lower for term in university_terms):
        return None
    ...
    # Question sur le RER
    elif any(term in question_lower for term in ["rer", "r.e.r", "transports"]):
        return f"Accès par le R.E.R. :\n{university_info['rer']}"

    # Question sur le train
    elif any(term in question_lower for term in ["train", "sncf", "ligne l", "transports"]):
        return f"Accès par le train :\n{university_info['train']}"

    # Question sur le bus
    elif any(term in question_lower for term in ["bus", "autobus", "transports"]):
        return f"Accès par le bus :\n{university_info['bus']}"

    # Question sur la voiture ou l'accès routier
    elif any(term in question_lower for term in
             ["voiture", "route", "autoroute", "a86", "a14", "stationnement", "parking"]):
        return f"Accès par la route :\n{university_info['route']}"

    # Question sur le vélo
    elif any(term in question_lower for term in ["vélo", "velo", "bicyclette", "veligo"]):
        return f"Accès en vélo :\n{university_info['velo']}"
    ...
    # Question générale sur l'université
    else:
...
# Modifier la fonction find_similar_question pour intégrer la nouvelle fonction
def find_similar_question(existing_data, question):
    normalized_question = question.lower().strip()
    current_keywords = set(extract_keywords(normalized_question))
    best_match = None
    ...
    # Vérifier d'abord si c'est une question sur les contacts de l'université
...
def ask_ollama_improved(context, question):
    is_association_question = any(term in question.lower() for term in ["association", "club", "asso", "activité étudiante", "existe-t-il"])
    is_student_life_question = any(term in question.lower() for term in ["aide", "soutien", "entraide", "vie étudiante", "solidarité", "accompagnement"])
    is_crous_question = any(term in question.lower() for term in ["crous", "resto", "restaurant", "cafétéria", "restauration", "repas"])
    is_transport_question = any(term in question.lower() for term in ["transport", "bus", "métro", "imagine r", "train", "rer", "navigo"])
    is_handicap_question = any(term in question.lower() for term in ["handicap", "sha", "accessibilité", "situation de handicap"])
...
def get_relevant_urls(urls, question, max_urls=5):
    ...
    url_categories = {
        ...
        "arts_martiaux": [url for url in urls if "combat" in url or "judo" in url or "boxe" in url or "self-defense" in url],
        "vie_etudiante": [url for url in urls if "solidarite" in url or "entraide" in url or "soutien" in url],
        "restauration": [url for url in urls if any(term in url.lower() for term in ["resto", "restaurant universitaire", "cafeteria", "repas"])],
        "vie etudiante": [url for url in urls if any(term in url.lower() for term in ["vie-etudiante", "suio", "service", "aide-a-la-vie"])],
    ...
    if "vie étudiante" in question_lower or "aide" in question_lower or "soutien" in question_lower:
        support_urls = []
        for url in urls:
            for assoc in aide_vie_etudiante_associations.keys():
                if assoc.lower() in url.lower():
                    support_urls.append(url)
                    break

        if support_urls:
            return support_urls[:max_urls]
    ...
    category_scores = {
        ...
        "general": 1  # Score minimal pour la catégorie générale
    }
...
def main():
    ...
    urls = {
        ...
        "https://www.crous-versailles.fr/contacts/bourses-et-aides-financieres/": ["bourses", "aides financières","CROUS", "précarité étudiante"],
        "https://www.crous-versailles.fr/contacts/social-et-accompagnement/": ["aide sociale", "accompagnement","CROUS", "solidarité étudiante"],
        "https://www.crous-versailles.fr/contacts/logement-et-vie-en-residence/": ["logement étudiant","vie en résidence", "CROUS","aide au logement"],
        "https://www.crous-versailles.fr/contacts/compte-izly/": ["compte Izly", "CROUS", "services étudiants","paiement universitaire"],
        ...
        "https://suaps.parisnanterre.fr/les-sports-et-activites/volley-ball": ["sport", "suaps", "volley-ball", "sport collectif"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/les-sports-individuels": ["sport", "suaps", "sport individuel"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/athletisme": ["sport", "suaps", "athlétisme", "sport individuel"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/escalade": ["sport", "suaps", "escalade", "sport individuel"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/tir-a-larc": ["sport", "suaps", "tir à l'arc", "sport individuel"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/les-sports-de-raquettes": ["sport", "suaps", "sport de raquettes"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/badminton": ["sport", "suaps", "badminton", "sport de raquettes"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/tennis": ["sport", "suaps", "tennis", "sport de raquettes"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/tennis-de-table": ["sport", "suaps", "tennis de table", "sport de raquettes"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/les-sports-de-combat": ["sport", "suaps", "sport de combat"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/jiu-jitsu": ["sport", "suaps", "jiu-jitsu", "sport de combat"],
        ...
        "https://suaps.parisnanterre.fr/les-sports-et-activites/mma-grappling": ["sport", "suaps", "mma", "sport de combat"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/self-defense": ["sport", "suaps", "self-defense", "sport de combat"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/education-corporelle-et-remise-en-forme": ["sport", "suaps", "éducation corporelle et remise en forme"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/education-posturale": ["sport", "suaps", "éducation posturale", "éducation corporelle et remise en forme"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/fitness": ["sport", "suaps", "fitness", "éducation corporelle et remise en forme"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/musculation": ["sport", "suaps", "musculation", "éducation corporelle et remise en forme"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/arts-du-mouvement": ["sport", "suaps", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/arts-du-cirque": ["sport", "suaps", "arts du cirque", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/atelier-choregraphie": ["sport", "suaps", "atelier chorégraphie", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/bachata": ["sport", "suaps", "bachata", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/danse-africaine": ["sport", "suaps", "danse africaine", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/danse-contemporaine": ["sport", "suaps", "danse contemporaine", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/zumba": ["sport", "suaps", "zumba", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/tango-argentin": ["sport", "suaps", "tango argentin", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/salsa": ["sport", "suaps", "salsa", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/rocknroll": ["sport", "suaps", "rock'n'roll", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/piloxing": ["sport", "suaps", "piloxing", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/kizomba": ["sport", "suaps", "kizomba", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/hip-hop": ["sport", "suaps", "hip-hop", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/danse-orientale": ["sport", "suaps", "danse orientale", "arts du mouvement"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/activites-nautiques": ["sport", "suaps", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/aquabike-aquagym-circuit-training": ["sport", "suaps", "aquabike", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/plongee": ["sport", "suaps", "plongée", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/natation-perfectionnement": ["sport", "suaps", "natation", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/natation-intermediaire": ["sport", "suaps", "natation", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/natation-competition": ["sport", "suaps", "natation", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/natation-apprentissage": ["sport", "suaps", "natation", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/bnssa": ["sport", "suaps", "BNSSA", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/baignade-libre": ["sport", "suaps", "baignade libre", "activités nautiques"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/activite-detente": ["sport", "suaps", "activité détente"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/yoga": ["sport", "suaps", "yoga", "activité détente"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/taichi-qi-gong": ["sport", "suaps", "taichi", "activité détente"],
        "https://suaps.parisnanterre.fr/les-sports-et-activites/relaxation": ["sport", "suaps", "relaxation", "activité détente"],
        "https://ufr-lce.parisnanterre.fr/associations" : ["association", "annuaire"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/dix-de-choeur": ["association","musique"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/melodix": ["association","musique"],
        "http://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/la-volt": ["association","musique"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/revolte-toi-nanterre": ["association", "éloquence et débat"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/les-unis-verts": ["association", "écologie"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/mun-society-paris-nanterre": ["association", "éloquence et débat"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/acfa": ["association","représentation étudiante"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/amnesty-international-groupe-jeunes-3047": ["association", "caritatif"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/faun": ["association","représentation étudiante"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/association-psychologie-du-developpement": ["association", "médias, lecture et écriture"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/les-indifferents": ["association", "théâtre"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/les-impunis-ligue-dimprovisation": ["association", "théâtre"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/eloquentia-nanterre": ["association", "éloquence et débat"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/lysias": ["association","éloquence et débat"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/lcc-production": ["association", "audiovisuel/cinéma"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/nuits-noires": ["association","audiovisuel/cinéma"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/atelier-decriture": ["association", "médias, lecture et écriture"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/lili-blooms-book-club": ["association", "médias, lecture et écriture"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/pile-a-lire": ["association","médias, lecture et écriture"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/rcva": ["association","culture scientifique"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/altiski": ["association","sport"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/cheerleading-paris-nanterre-1": ["association", "sport"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/laocho": ["association","sport"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/la-nav-nanterre-association-de-voile": ["association", "sport"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/aumonerie-catholique-des-etudiant-es": ["association", "solidarité et entraide"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/asega": ["association","solidarité et entraide"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/cercle-marxiste-de-nanterre": ["association", "citoyenneté"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/etudiants-musulmans-de-france-nanterre": ["association", "solidarité et entraide"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/ucph": ["association","solidarité et entraide"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/union-etudiants-juifs-france-nanterre": ["association", "solidarité et entraide"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/lathena": ["association","caritatif"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/antenne-jeunes-unicef-nanterre": ["association", "caritatif"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/amicale-des-etudiant-es-senegalais-es": ["association", "cultures du monde"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/compagnie-ptdr": ["association", "théâtre"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/paris-nanterre-maroc-1": ["association", "cultures du monde"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/le-poing-leve": ["association","citoyenneté"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/union-etudiante-nanterre": ["association", "représentation étudiante"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/unef-nanterre": ["association","représentation étudiante"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/ugen-fse": ["association","représentation étudiante"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/promet": ["association","association de filiere, ssa, sciences sociales et administrations"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/hypothemuse": ["association","association de filiere, ssa, sciences sociales et administrations"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/gang": ["association","association de filiere, ssa, sciences sociales et administrations"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/enape": ["association","association de filiere, ssa, sciences sociales et administrations"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/bde-staps-rhinos": ["association", "sciences et techniques des activites physiques et sportives (staps)"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/psychx": ["association","sciences psychologiques et sciences de l'éducation (spse)"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/comite-dactions-et-reseau-des-etudiants-en-sante-et-societe": ["association", "sciences psychologiques et sciences de l'éducation (spse)"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/les-alhumes": ["association","sciences psychologiques et sciences de l'éducation (spse)"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/cine-rebelle": ["association","philosophie, information-communication, langage, littérature, arts du spectacle (phillia)"],
        "https://aca2.parisnanterre.fr/associations/annuaire-des-associations-etudiantes/association-west-street": ["association", "sciences economiques, gestion, mathematiques, infomatique (segmi)"],
    }
    while True:
